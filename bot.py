...
def get_buttons(channels):
    ...
    for i, ch in enumerate(channels, 1):
        kb.append([InlineKeyboardButton(f"📢 {i}-Kanal", url=f"https://t.me/{ch[1:]}")])
...
async def start(update, context):
    ...
    if ok:
        await update.message.reply_text("Marxamat film Kodini yuboring ! 🔰")
    else:
        await update.message.reply_text(
            "Film ko'rish uchun obuna bo'ling ! 👇🏻🎬",
...
async def check_callback(update, context):
    ...
    ok, nots = await check_sub(uid, context)
    if ok:
        await q.edit_message_text("Marxamat film Kodini yuboring ! 🔰")
    else:
...
async def film(update, context):
    ...
    if kod in FILMS:
        ...
        await update.message.reply_text("Maroqli Tomosha 🍿")
    else:
